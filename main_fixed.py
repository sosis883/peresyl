...
# Ссылки для кнопок
URL_HOW_TO = "https://ВСТАВЬ_ССЫЛКУ_СЮДА"  # Как брать задания (не используется в кнопках)
URL_SUPPORT = "https://ВСТАВЬ_ССЫЛКУ_СЮДА"  # Поддержка (не используется в кнопках)
URL_REFERRAL = "https://ВСТАВЬ_ССЫЛКУ_СЮДА"  # Реф. программа (не используется в кнопках)

# НОВЫЕ ССЫЛКИ ДЛЯ КНОПОК В КАНАЛЕ
URL_PAYMENTS = "https://ВСТАВЬ_ССЫЛКУ_ВЫПЛАТ_СЮДА"  # Выплаты
URL_TRAINING = "https://ВСТАВЬ_ССЫЛКУ_ОБУЧЕНИЯ_СЮДА"  # Обучение
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_allowed(uid):
        await update.message.reply_text(
            "У вас нет доступа к боту.\n\nОбратитесь к @galxar для покупки доступа."
        )
        return ConversationHandler.END
    await update.message.reply_text(
...
async def confirm_task(query, context):
    ...
    # Формируем текст для ЛС (теперь Galxar)
    ...
    # 2. Выплаты - канал Galxar
    # 3. Обучение - канал Galxar
    buttons = [
...
async def run_bot():
    ...
    @fastapi_app.get("/")
    async def root():
        return {"status": "ok", "bot": "GalxarBot", "mode": "webhook"}
